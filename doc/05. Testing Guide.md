...
This guide covers all implemented endpoints across Phase 2 (OAuth 2.0 Core) and Phase 3 (OIDC Layer). For unit test commands, see each module's `src/test` directory.
...
# 3. Seed a test client (see Client Registration below)
...
- Old refresh token is **atomically revoked** on use
- Reusing the old refresh token → `invalid_grant` + both tokens in that family are revoked
